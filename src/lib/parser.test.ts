...
const pageA = readFile(
  new URL("../../seed/dogs-page-A.html", import.meta.url),
  "utf8",
);
const pageB = readFile(
  new URL("../../seed/dogs-page-B.html", import.meta.url),
  "utf8",
);
...
test("offline fixture parsing returns a complete roster", async () => {
  ...
  assert.ok(
    companions.every(
      (companion) => companion.name && companion.photoUrls.length > 0,
    ),
  );
  assert.equal(
    companions.find((companion) => companion.name === "Hattie")?.adopted,
    false,
  );
  assert.equal(
    companions.find((companion) => companion.name === "Charlie")?.adopted,
    true,
  );
  assert.equal(
    companions.find((companion) => companion.name === "Jack")?.adopted,
    true,
  );
  assert.equal(
    companions.find((companion) => companion.name === "Romulus")?.adopted,
    true,
  );
});

test("a hand-typed adoption marker updates the parsed record", async () => {
  const companions = await parseCompanionRoster(await pageB, {
    deterministic: true,
  });
...
test("wrapped cards and non-breaking spaces parse like plain sections", async () => {
  ...
  assert.deepEqual(companions[0].photoUrls, [
    "https://example.test/juniper.png",
  ]);
});
