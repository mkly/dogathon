...
async function getOrganizationContext(
  ...
  return permitted
    ? organizationContext(resolvedSession.user.id, membership, orgId)
    : null;
}
...
export async function requireApiOrganization(
  ...
  { ok: false; response: Response } | { ok: true; context: OrganizationContext }
> {
  const slug = requestHeaders.get("x-organization-slug");
  if (slug) {
    const access = await dependencies.getAccessBySlug(
      requestHeaders,
      slug,
      permission,
    );
...
export function forOrganization<T extends object>(
  orgId: string,
  where?: T,
): T & { orgId: string } {
