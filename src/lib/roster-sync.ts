...
export async function syncRoster(
  ...
  if (!settings.sourceUrl) {
    throw new Error(
      "Roster sync needs an adoption-page source URL; save one in staff settings first",
    );
  }
  ...
  const companions = onlyIdentifyingSourceUrls(
    await parseRosterDocuments(
      documents ?? [{ text, sourceUrl: source }],
      options.signal,
    ),
  );
  options.signal?.throwIfAborted();
  ...
  if (companions.length === 0) {
    throw new Error(
      "Roster sync refused to mark every resident unavailable after parsing an empty roster",
    );
  }
  ...
  const summary = await prisma.$transaction(
    async (tx) => {
      ...
      const existingSourceUrls = new Set(
        before.map((resident) => resident.sourceUrl).filter(Boolean),
      );
      const { adoptedCandidates, unavailableCandidates, availableCandidates } =
        planRosterAvailabilityChanges(before, companions, {
          usedFallbackCapture,
          rosterComplete,
        });
      ...
      return {
        created: companions.filter(
          (companion) =>
            !existingNames.has(companion.name) &&
            !existingSourceUrls.has(
              normalizeSourceUrl(companion.sourceUrl ?? ""),
            ),
        ).length,
        updated: companions.filter(
          (companion) =>
            existingNames.has(companion.name) ||
            existingSourceUrls.has(
              normalizeSourceUrl(companion.sourceUrl ?? ""),
...
async function retireResident(
  ...
  const heroPhotoUrl =
    latestPhoto?.webUrl ?? latestPhoto?.url ?? resident.photoUrls[0] ?? null;
...
export function planRosterAvailabilityChanges<
  T extends ResidentAvailabilitySnapshot,
>(
  before: T[],
  companions: CompanionRecord[],
  source: Pick<RosterSource, "usedFallbackCapture" | "rosterComplete">,
) {
  const onRoster = (resident: T) =>
    companions.some((companion) =>
      companionMatchesResident(companion, resident),
    );
  const explicitlyMarked = (resident: T) =>
    companions.some(
      (companion) =>
        companion.adopted && companionMatchesResident(companion, resident),
    );
  const absenceIsReliable =
    !source.usedFallbackCapture && source.rosterComplete;
  ...
  const availableCandidates = absenceIsReliable
    ? before.filter(
        (resident) =>
          !resident.available &&
...
function companionMatchesResident(
  companion: CompanionRecord,
  resident: ResidentAvailabilitySnapshot,
): boolean {
...
export function assertPlausibleUnavailableCount(
  ...
  if (
    unavailableCandidates / availableResidents >
    MAX_LIVE_UNAVAILABLE_FRACTION
  ) {
...
async function parseRosterDocuments(
  ...
  const worker = async () => {
    while (next < documents.length) {
      ...
      parsed[index] = (await parseCompanionRoster(document.text)).map(
        (companion) => ({
          ...companion,
  ...
  await Promise.all(
    Array.from(
      { length: Math.min(ROSTER_DOCUMENT_CONCURRENCY, documents.length) },
      worker,
    ),
  );
  return parsed.flat();
 ...
 */
export function onlyIdentifyingSourceUrls(
  companions: CompanionRecord[],
): CompanionRecord[] {
  ...
  for (const companion of companions) {
    if (companion.sourceUrl) {
      pageCounts.set(
        companion.sourceUrl,
        (pageCounts.get(companion.sourceUrl) ?? 0) + 1,
      );
    }
  }
...
export async function upsertCompanions(
  ...
  const existingSlugs = new Set(
    existingResidents.map((resident) => resident.slug),
  );
  const residentsByName = new Map(
    existingResidents.map((resident) => [resident.name, resident]),
  );
  const residentsBySource = new Map(
    existingResidents
  ...
  const companionsWithSlugs = companions.map((companion) => {
    const sourceUrl = normalizeSourceUrl(companion.sourceUrl ?? "");
    const existing =
      (sourceUrl ? residentsBySource.get(sourceUrl) : undefined) ??
  ...
  for (
    let offset = 0;
    offset < companionsWithSlugs.length;
    offset += RESIDENT_WRITE_BATCH_SIZE
  ) {
    signal?.throwIfAborted();
    await Promise.all(
      companionsWithSlugs
        .slice(offset, offset + RESIDENT_WRITE_BATCH_SIZE)
        .map(({ companion, slug }) =>
          upsertCompanion(tx, orgId, companion, slug, liveSource),
        ),
    );
  }
}
...
export async function loadRoster(
  ...
  if (localPath) {
    const text = await readFile(localPath, {
      encoding: "utf8",
      signal: options.signal,
    });
    return {
  ...
  const captureText = await readFile(seedCapturePath(capture), {
    encoding: "utf8",
    signal: options.signal,
  });
  return {
...
type RosterToolName =
  ...
  | "firecrawl_crawl";
const FETCHING_TOOLS = new Set<RosterToolName>([
  "firecrawl_scrape",
  "firecrawl_batch_scrape",
  "firecrawl_crawl",
]);
const BULK_TOOLS = new Set<RosterToolName>([
  "firecrawl_batch_scrape",
  "firecrawl_crawl",
]);
type FirecrawlCaller = (
  name: RosterToolName,
...
export async function discoverRosterWithCompleteness(
  ...
  const firecrawl =
    options.firecrawl ??
    ((name, input) =>
      requestFirecrawl(name, input, { sourceUrl, signal: options.signal }));
  ...
  const log =
    options.log ??
    ((message: string) => console.info(`[roster-sync] ${message}`));
  let toolCalls = 0;
  ...
  const saveProvisionalNotes = async (
    name: RosterToolName,
    input: Record<string, unknown>,
    outcome: string,
  ) => {
    ...
    try {
      ...
      log(
        `saving provisional notes failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
  const executeTool = async (
    ...
    try {
      if (toolCalls >= MAX_ROSTER_TOOL_CALLS) {
        throw new Error(
          `Roster discovery is limited to ${MAX_ROSTER_TOOL_CALLS} Firecrawl calls`,
        );
      }
      toolCalls += 1;
      attemptedName = name;
      ...
      if (BULK_TOOLS.has(name)) {
        await saveProvisionalNotes(
          name,
          input,
          "the fetch was issued but that sync did not record its result",
        );
      }
      ...
      if (FETCHING_TOOLS.has(name)) {
        const requestedUrls =
          name === "firecrawl_batch_scrape" && Array.isArray(input.urls)
            ? [
                ...new Set(
                  input.urls.filter(
        ...
        else if (isListingFetch(sourceUrl, input))
          listingDocuments.push(...sourceDocuments);
      ...
      if (BULK_TOOLS.has(name)) {
        ...
        log(
          `${describe} ${completeness.status}: ${completeness.completed}/${completeness.total} pages in ${elapsedSeconds(startedAt)}s`,
        );
        await saveProvisionalNotes(
          name,
          input,
          `the fetch finished with status ${completeness.status}, ${completeness.completed} of ${completeness.total} pages`,
        );
      } else {
      ...
      if (name === "firecrawl_scrape")
        return summarizeScrapeToolResult(result, sourceUrl);
  ...
  const tools = {
    firecrawl_map: tool({
      description:
        "Find pages on the rescue website that may contain the adoptable companion roster.",
      inputSchema: z.object({
        url: z.string().describe("The rescue website URL to map."),
        search: z
          .string()
          .optional()
          .describe("Optional terms such as adoptable companions."),
        ...
        loadMore: z
          .object({
            selector: z
              .string()
              .min(1)
              .describe("CSS selector for the Show More or Load More control."),
            maxClicks: z
              .number()
              .int()
              .min(1)
              .optional()
              .describe(
                `Number of clicks to attempt, capped at ${MAX_FIRECRAWL_LOAD_MORE_CLICKS}.`,
              ),
          })
          .optional(),
      }),
    ...
    firecrawl_batch_scrape: tool({
      description:
        "Fetch a chosen set of pages as markdown for roster parsing: the companion detail pages and further listing pages picked from a scraped listing's links. Only these exact URLs are fetched.",
      inputSchema: z.object({
        urls: z
          .array(z.string())
          .min(1)
          .max(MAX_BATCH_SCRAPE_URLS)
          .describe(
            "The exact page URLs to fetch, chosen from the listing's links.",
          ),
      }),
    ...
    firecrawl_crawl: tool({
      description:
        "Last resort when a listing's links cannot be read: crawl the listing path, following pagination and companion links found under it. The crawl never leaves the configured listing path.",
      inputSchema: z.object({
        ...
        limit: z
          ...
          .int()
          .min(1)
          .max(MAX_FIRECRAWL_CRAWL_PAGES)
          .optional(),
        maxDiscoveryDepth: z
          .number()
          .int()
          .min(0)
          .max(MAX_FIRECRAWL_DISCOVERY_DEPTH)
          .optional(),
      }),
    ...
    save_sync_notes: tool({
      description:
        "Save notes for the next sync of this source: the listing URL to scrape, how its companion detail links and pagination links look, how many companions the listing showed, data endpoints and their parameters, and any site quirks. Replace the previous notes entirely.",
      inputSchema: z.object({
  ...
  const rosterDocuments =
    companionDocuments.length > 0 ? companionDocuments : listingDocuments;
  log(
    `discovery finished after ${steps.length} model steps, ${toolCalls} tool calls, ${rosterDocuments.length} roster documents in ${elapsedSeconds(discoveryStartedAt)}s`,
  );

  if (rosterDocuments.length === 0) {
    throw new Error(
      "Roster discovery completed without scraping roster content",
    );
  }
  return {
    text: rosterDocuments
  ...
  const settings =
    name === "firecrawl_batch_scrape"
      ...
      : [
          ...
          input.maxDiscoveryDepth === undefined
            ? null
            : `maxDiscoveryDepth ${String(input.maxDiscoveryDepth)}`,
        ]
          .filter(Boolean)
          .join(", ");
  ...
  return urls
    ...
    .map((url) => {
      if (typeof url !== "string")
        throw new Error("Roster tool URL must be a string");
      return url;
    });
}
...
function describeToolInput(input: Record<string, unknown>): string {
  ...
  const loadMoreDescription =
    loadMore === undefined ? "" : describeLoadMore(loadMore);
  const urlList = Array.isArray(urls)
    ? `${urls.length} urls (${urls.slice(0, 3).map(String).join(", ")}${urls.length > 3 ? ", ..." : ""})`
    : "";
  return [
...
function readCrawlCompleteness(value: unknown): RosterCompleteness {
  ...
  const detail =
    ...
    !Array.isArray(record.completeness)
      ...
      : {};
  const completed = finiteNonNegativeInteger(
    detail.completed ?? record.completed,
    0,
  );
  const total = finiteNonNegativeInteger(
    detail.total ?? record.total,
    completed,
  );
  return {
...
function combineCrawlCompleteness(
  crawls: RosterCompleteness[],
): RosterCompleteness {
  if (crawls.length === 0) {
    return {
      complete: false,
...
export async function requestFirecrawl(
  ...
  const enabled =
    options.apiKey === undefined ? env.features.firecrawl : Boolean(apiKey);
  if (!enabled || !apiKey) {
    throw new Error("FIRECRAWL_API_KEY is required to fetch a live roster");
  }
  const baseUrl = (options.baseUrl ?? env.FIRECRAWL_BASE_URL).replace(
    /\/+$/u,
    "",
  );
  ...
  if (name === "firecrawl_crawl")
    return requestFirecrawlCrawl(input, jobOptions);
  if (name === "firecrawl_batch_scrape")
    return requestFirecrawlBatchScrape(input, jobOptions);
  ...
  if (!response.ok || payload.success === false) {
    throw new Error(
      payload.error ??
        `Firecrawl ${endpoint} request failed (${response.status})`,
    );
  }
  return payload;
}

function firecrawlScrapeBody(
  input: Record<string, unknown>,
  ...
  const actions =
    loadMore === undefined
      ? []
      : Array.from({ length: loadMoreClickCount(loadMore) }, () => [
...
function loadMoreSelector(value: unknown): string {
  ...
  if (!selector.trim())
    throw new Error("Roster scrape loadMore selector must not be empty");
  return selector;
}

function loadMoreClickCount(value: unknown): number {
  if (!value || typeof value !== "object")
    throw new Error("Roster scrape loadMore must be an object");
  const maxClicks = (value as Record<string, unknown>).maxClicks;
  if (maxClicks === undefined) return 1;
  if (
    typeof maxClicks !== "number" ||
    !Number.isFinite(maxClicks) ||
    maxClicks < 1
  ) {
    throw new Error(
      "Roster scrape loadMore maxClicks must be a positive number",
    );
  }
...
async function requestFirecrawlCrawl(
  input: Record<string, unknown>,
  options: Required<
    Pick<FirecrawlRequestOptions, "apiKey" | "baseUrl" | "fetch">
  > &
    FirecrawlRequestOptions,
): Promise<FirecrawlCrawlResult> {
  const sourceUrl = options.sourceUrl;
  if (!sourceUrl)
    throw new Error("A source URL is required to scope a Firecrawl crawl");
  ...
  const requestedStartsInScope = pathMatchesScope(
...
async function requestFirecrawlBatchScrape(
  ...
  if (!sourceUrl)
    throw new Error(
      "A source URL is required to scope a Firecrawl batch scrape",
    );
  ...
  if (input.urls.length > MAX_BATCH_SCRAPE_URLS) {
    throw new Error(
      `Roster batch scrape accepts at most ${MAX_BATCH_SCRAPE_URLS} URLs`,
    );
  }
  const urls = [
    ...new Set(
...
async function submitFirecrawlJob(
  ...
  if (!submitResponse.ok || submit.success === false || !submit.id) {
    throw new Error(
      submit.error ??
        `Firecrawl ${endpoint} submission failed (${submitResponse.status})`,
    );
  }

  const timeoutMs = Math.max(
    0,
    options.crawlTimeoutMs ?? FIRECRAWL_CRAWL_TIMEOUT_MS,
  );
  const pollIntervalMs = Math.max(
    0,
    options.pollIntervalMs ?? FIRECRAWL_POLL_INTERVAL_MS,
  );
  ...
  while (Date.now() < deadline) {
    ...
    if (!response.ok || payload.success === false) {
      throw new Error(
        payload.error ??
          `Firecrawl ${endpoint} status failed (${response.status})`,
      );
    }
...
export function assertRelatedUrl(sourceUrl: string, candidate: unknown) {
  ...
  if (requested.protocol !== "http:" && requested.protocol !== "https:") {
    throw new Error(
      `Roster tool refused unsupported protocol: ${requested.protocol}`,
    );
  }
  if (
    !isPublicHttpUrl(source.toString()) ||
    !isPublicHttpUrl(requested.toString())
  ) {
    throw new Error(
      `Roster tool refused private or non-public host: ${requested.hostname}`,
    );
  }
  if (!isRelatedHost(source.hostname, requested.hostname)) {
    throw new Error(
      `Roster tool refused unrelated host: ${requested.hostname}`,
    );
  }
}
...
function summarizeScrapeToolResult(value: unknown, sourceUrl: string): string {
  ...
  const data =
    ...
    !Array.isArray(record.data)
      ...
      : record;
  const links = sameSiteUrls(
    Array.isArray(data.links) ? data.links : [],
    sourceUrl,
  );
  const json = scrapedJson(data);
  const jsonLinks =
    json === null ? [] : sameSiteUrls(stringValues(json), sourceUrl);
  const dataEndpoints = dataEndpointsFromHtml(
  ...
  return truncateToolResult(
    {
      markdown:
        markdown.length <= MAX_SCRAPE_MARKDOWN_CHARS
          ? markdown
      ...
      dataEndpointsOmitted: Math.max(
        0,
        dataEndpoints.length - MAX_SCRAPE_DATA_ENDPOINTS,
      ),
    },
...
function dataEndpointsFromHtml(html: string, sourceUrl: string): string[] {
  ...
  const quotedValues = scriptContents.flatMap((script) =>
    [...script.matchAll(/(["'])(.*?)\1/gu)].map(
      (match) => match[2]?.replaceAll("\\/", "/") ?? "",
    ),
  );
  return sameSiteUrls(quotedValues, sourceUrl).filter((value) =>
    isDataEndpoint(new URL(value)),
  );
}
...
function isListingFetch(
  ...
  if (withoutTrailingSlash(url) === withoutTrailingSlash(sourceUrl))
    return true;
  try {
...
function isDataEndpoint(url: URL): boolean {
  return (
    /\/(?:wp-json|admin-ajax(?:\.php)?|api|graphql)(?:\/|$)/iu.test(
      url.pathname,
...
function extractScrapedDocuments(
  value: unknown,
  fallbackUrls: string[],
): RosterDocument[] {
  const record =
  ...
  return values.flatMap((item, index) => {
    const sourceUrl =
      scrapedSourceUrl(item) ?? fallbackUrls[index] ?? fallbackUrls[0] ?? "";
...
function scrapedSourceUrl(value: unknown): string | null {
  ...
  for (const candidate of [
    ...
    if (typeof candidate === "string" && /^https?:\/\//iu.test(candidate))
      return candidate;
  }
  return null;
}
...
function fallbackCapture(sourceUrl: string): string {
  return /dogs-page-B\.html/i.test(sourceUrl)
    ? "dogs-page-B.html"
    : DEFAULT_CAPTURE;
}
