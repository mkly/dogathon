...
test("round-trips a signed sponsorship id into the public selection URL", () => {
  const url = new URL(sponsorshipSelectionUrl(
    "https://pawcast.test",
    "huffy puff",
    sponsorshipId,
    secret,
  ...
  assert.deepEqual(
    verifySponsorshipSelectionToken(url.searchParams.get("token")!, secret, now),
    { sponsorshipId },
  );
});
...
test("rejects tampered, malformed, and expired tokens", () => {
  ...
  assert.equal(verifySponsorshipSelectionToken("not-a-token", secret, now), null);
  assert.equal(verifySponsorshipSelectionToken(
    token,
    secret,
