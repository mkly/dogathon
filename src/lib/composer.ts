    ...
    messages: [
      {
        role: "system",
        content:
          `You write warm, short email updates in an animal shelter's voice. Use only facts in the supplied JSON; never invent details.${regularUpdateGuidance} Format the notes section with the literal Markdown heading "## Recent notes". Return only a JSON object with subject and bodyText strings.`,
      },
      {
        role: "user",
