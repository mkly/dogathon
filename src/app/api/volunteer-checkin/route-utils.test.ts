...
const input = {
  messages: [
    {
      id: "message-1",
      parts: [{ type: "text" as const, text: "A calm walk." }],
      role: "user" as const,
    },
  ],
  orgSlug: "huffy-puff",
...
test("returns 401 when the interview request is unauthenticated", async () => {
  const result = await routeFor(false)(
    new Request("http://localhost/api/volunteer-checkin/chat"),
    input,
  );
...
test("returns 403 when the interview user is not an organization member", async () => {
  const result = await routeFor(true)(
    new Request("http://localhost/api/volunteer-checkin/chat"),
    input,
  );
  assert.equal(result.ok, false);
...
test("accepts only the next user message after the stored transcript", async () => {
  const stored = [
    ...
    {
      id: "a0",
      parts: [{ type: "text" as const, text: "How was her mood?" }],
  ...
  const nextInput = {
    ...
    messages: [
      ...
      {
        id: "u1",
        parts: [{ type: "text" as const, text: "She was happy." }],
        role: "user" as const,
      },
    ],
  };
  ...
  const replaced = await route(request, {
    ...nextInput,
    messages: [nextInput.messages[2]],
  });
...
test("accepts an empty opening turn only for an empty check-in with a photo", async () => {
  ...
  assert.equal(
    (await openingRoute("orgs/org-1/photo.jpg")(request, openingInput)).ok,
    true,
  );
