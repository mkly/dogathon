...
import {
  SPONSOR_UPDATE_BODY_MAX_LENGTH,
  sponsorUpdateBodyOverLimitMessage,
} from "@/lib/sponsor-update-body";
...
const trimmedDraftSchema = draftInputSchema.transform(
  ({ subject, teaser, bodyText }) => ({
...
export async function PATCH(request: Request, { params }: RouteContext) {
  ...
  if (!validated.success) {
    return Response.json(
      { error: "Subject, teaser, and body are required" },
      { status: 400 },
    );
  }
  if (
    !validated.data.subject ||
    !validated.data.teaser ||
    !validated.data.bodyText
  ) {
    return Response.json(
      { error: "Subject, teaser, and body cannot be empty" },
      { status: 400 },
    );
  }
  if (validated.data.teaser.length > 240) {
    return Response.json(
      { error: "Teaser cannot exceed 240 characters" },
      { status: 400 },
    );
  }
  if (validated.data.bodyText.length > SPONSOR_UPDATE_BODY_MAX_LENGTH) {
    return Response.json(
      { error: sponsorUpdateBodyOverLimitMessage() },
      { status: 400 },
    );
  }
  ...
  if (updated.count !== 1) {
    const exists = await prisma.sponsorUpdate.findFirst({
      where: { id, orgId },
      select: { id: true },
    });
    return Response.json(
