  ...
  function discard(checkIn: UnfinishedCheckIn) {
    ...
    startTransition(async () => {
      try {
        ...
        pushToast("success", `Removed the unfinished update for ${checkIn.resident.name}.`);
      } catch {
        setRemoved((current) => current.filter((id) => id !== checkIn.id));
        pushToast("error", `The update for ${checkIn.resident.name} could not be removed.`);
      }
    });
  }
  ...
  return (
    <>
      ...
      <ul aria-label="Unfinished updates" className={styles.resumeList}>
        {visible.map((checkIn) => (
          <li className={clsx(felt["felt-button"], "felt-oatmeal", styles.resumeRow)} key={checkIn.id}>
