...
async function requireVolunteerAccess(orgSlug: string, nextPath: string) {
  ...
  const access = await getOrganizationAccessBySlug(
    await headers(),
    parsed.data,
    {
      roster: ["contribute"],
  ...
  if (deleted.count === 1 && photos.length > 0) {
    ...
    await Promise.all(
      photos.flatMap((photo) =>
        [photo.storageKey, photo.webStorageKey]
          ...
          .map(async (key) => {
            try {
              ...
              console.error(
                "volunteer photo could not be removed from storage",
                error,
              );
            }
          }),
...
export async function finishCheckIn(
  ...
  const access = await requireVolunteerAccess(
    orgSlug,
    sessionUrl(orgSlug.trim(), checkInId.data),
  );
  ...
  if (!rateLimit.allowed)
    redirect(sessionUrl(access.orgSlug, checkInId.data, "rate-limited"));
  ...
  const transcript = interviewTranscriptSchema
    .min(1)
    .max(MAX_INTERVIEW_MESSAGES)
    .safeParse(rawMessages);
  if (
    !transcript.success ||
    !transcript.data.some((message) => message.role === "user")
  ) {
