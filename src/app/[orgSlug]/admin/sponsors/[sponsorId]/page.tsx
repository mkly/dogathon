...
export default async function SponsorDetailPage({
  ...
  const canManageSponsorships = await checkOrganizationPermission(
    requestHeaders,
    context.orgId,
    {
    ...
    <PageViewTransition>
      <AdminPage variant="directory">
        ...
        {ongoingSponsorships.length > 0 ? (
          <section
            aria-labelledby="manage-sponsorships-heading"
            className={styles.awaitingSection}
          >
            <div className={styles.historyTitle}>
              <h2 id="manage-sponsorships-heading">Manage sponsorships</h2>
              <AdminBadge tone="brick">
                {ongoingSponsorships.length} ongoing
              </AdminBadge>
            </div>
            <p className={styles.limitNotice}>
              Sponsorships keep renewing while sponsors are between companions.
              Staff can move an active sponsorship or one choosing a next
        ...
        <section aria-labelledby="history-heading">
              ...
              <tbody>
                {sponsor.sponsorships.map((sponsorship) => (
                  <tr key={sponsorship.id}>
                    ...
                    <td>
                      {sponsorship.endedAt
                        ? formatDate(sponsorship.endedAt)
                        : "—"}
                    </td>
                    <td className={styles.reason}>
                      {sponsorshipEndedReasonLabel(sponsorship.endedReason)}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {sponsor._count.sponsorships > SPONSORSHIP_HISTORY_LIMIT ? (
            <p className={styles.limitNotice}>
              Showing the first {SPONSORSHIP_HISTORY_LIMIT} sponsorships in this
              history.
            </p>
          ) : null}
        </section>
