...
export const metadata: Metadata = {
  ...
  description: "Chip in a little each month for a companion at a rescue, and hear how they are doing from the people who spend time with them.",
};
...
const questions = [
  {
    ...
    answer: "To the rescue. It helps cover the everyday cost of looking after their residents, from food and bedding to trips to the vet. Each rescue sets its own amounts, and you will see them on the companion's page.",
  },
  {
    question: "Is sponsoring the same as adopting?",
    answer: "No. Sponsoring supports a companion while the rescue looks after them. You do not need to live nearby or be ready to adopt. If you would like to adopt, the rescue can tell you how.",
  },
  {
      ...
      <main className={styles.shell}>
        ...
        <header className={styles.header}>
          <Link aria-label="Pawcast home" className={styles.home} href="/">
            <Image alt="Pawcast" className={styles.wordmark} preload src={wordmark} />
          </Link>
          <nav aria-label="Main" className={styles.nav}>
            <a href="#how-it-works">How it works</a>
            <a href="#rescues">Rescues</a>
            <Link className={styles.accountLink} href="/account">
              <StitchBadge tone="cream"><Stitch fine />My sponsorship</StitchBadge>
            </Link>
          </nav>
        </header>
        ...
        <div id="content">
          <FeltPanel className={styles.hero} tone="moss">
            <div className={styles.heroCopy}>
              ...
              <h1>
                Stand by a <span className="felt-hl">rescue companion</span> until they are home.
              </h1>
              <p className={styles.lede}>
                Pick a companion at a rescue, chip in a few dollars a month, and hear how they are doing
                from the volunteers who see them every week.
              </p>
              <FeltLink className={styles.heroLink} href="#rescues" tone="mustard">
                Meet the companions
              </FeltLink>
            </div>
          ...
          <section aria-labelledby="how-title" className={styles.how} id="how-it-works">
            ...
            <ol className={styles.steps}>
              {steps.map((step, index) => (
                <li key={step.title}>
                  <FeltPanel className={styles.step} stitched={false} tone="cream">
                    <StitchBadge className={styles.stepNumber} tone="denim">{index + 1}</StitchBadge>
                    <h3>{step.title}</h3>
                    <p>{step.copy}</p>
                  </FeltPanel>
                </li>
          ...
          <section aria-labelledby="rescues-title" className={styles.rescues} id="rescues">
            <div className={styles.rescueIntro}>
              <h2 id="rescues-title">Start with a rescue.</h2>
              <p>
                Every rescue here has people who turn up for their residents every day.
                Pick one and meet the companions in their care.
              </p>
              <ul className={styles.rescueList}>
                {organizations.length ? organizations.map((organization) => (
                  <li key={organization.id}>
                    <FeltLink className={styles.rescueLink} href={`/${organization.slug}`} tone="oatmeal">
                      <span>{organization.name}</span>
                      <small>Meet their companions</small>
                    </FeltLink>
                  </li>
                )) : <li className={styles.rescueEmpty}>The first rescues are settling in. Check back soon.</li>}
              </ul>
            </div>

            {companion && featuredOrganization ? (
              <FeltPanel className={styles.featured} stitched={false} tone="oatmeal">
                <PhotoPatch
                ...
                <div className={styles.featuredCopy}>
                  <p className={styles.featuredLabel}>Waiting at {featuredOrganization.name}</p>
                  <h3>{companion.name}</h3>
                  <p>{companion.breed} · {companion.ageText}</p>
                  <FeltLink
