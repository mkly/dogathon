...
function isReservedSlug(error: unknown): error is APIError {
  return (
    ...
    error.body?.code === RESERVED_ORGANIZATION_SLUG_ERROR
  );
}
...
export async function createOrganization(
  ...
  if (!input.success)
    return { error: "Enter a rescue name and a valid organization slug." };
  ...
  try {
    ...
    if (isReservedSlug(error)) {
      return {
        error:
          error.body?.message ??
          "That organization slug is reserved. Choose another slug.",
      };
    }
    if (isSlugCollision(error)) {
      return {
        error: "That organization slug is already taken. Choose another slug.",
      };
    }
    return {
      error:
        "We could not create that organization. Check the details and try again.",
    };
  }
...
export async function setActiveOrganization(formData: FormData) {
  const requestHeaders = await headers();
  const input = organizationSelectionSchema.safeParse(
    Object.fromEntries(formData),
  );
  if (!input.success)
    redirect("/staff/organizations?error=invalid-organization");
