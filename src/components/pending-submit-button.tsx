...
export function PendingAdminSubmitButton({
  children,
  pendingLabel,
  ...props
}: AdminButtonProps & PendingLabels) {
  const { pending } = useFormStatus();
  return (
    <AdminButton
      {...props}
      disabled={pending || props.disabled}
      style={{ minWidth: "12ch", ...props.style }}
    >
...
export function PendingFeltSubmitButton({
  children,
  pendingLabel,
  ...props
}: FeltButtonProps & PendingLabels) {
  const { pending } = useFormStatus();
  return (
    <FeltButton
      {...props}
